...
use libs::{Library, update_queue, Data};
        ...
        if let Event::Input(input) = events.next()? {
            match input {
                ...
                Key::Char('d') => {
                    if data.tabindex == 2 {
                        client
                            .delete(data.queue.state.selected().unwrap_or(0) as u32 + (data.drained + 1) as u32)
                            .unwrap_or(());
                        update_queue(&mut data, &mut client);
                ...
                Key::Char('\n') => {
                    ...
                    if data.tabindex == 2 {
                        client.switch(data.queue.state.selected().unwrap() as u32 + data.drained as u32)?;
                        client.pause(true)?;
                        client.play()?;
                    }
                    if data.tabindex == 1 && !data.options {
                        ...
                        match data.library.items[idx].tag {
                            LibKind::Home => {
                                match data.library.state.selected().unwrap_or(0) {
                            ...
                            LibKind::Playlist => {
                                ...
                                //data.library = 0;
                                for play in p_list {
                                    data.library.push(LibItem::new(play.title.unwrap_or("".into()), LibKind::Title))
                                }
                            }
                            LibKind::Option => {
                                match data.library.state.selected().unwrap_or(0) {
